...
#include <react-native-bls-signatures/elements.hpp>
...
template <typename V, typename C, typename F> std::vector<V> convert_container(C const& c, F&& f)
{
    std::vector<V> result;
...
PublicKey public_key_from_g1(bls::G1Element const& g1) { return utils::bytes_cast<Key::PUB_KEY_LEN>(g1.Serialize()); }
...
Signature signature_from_g2(bls::G2Element const& g2) { return utils::bytes_cast<Key::SIG_LEN>(g2.Serialize()); }
...
{
    return bls::AugSchemeMPL().Verify(
        adapters::public_key_to_g1(public_key), message, adapters::signature_to_g2(signature));
}

PublicKey Key::AggregatePublicKeys(std::vector<PublicKey> const& public_keys)
{
    std::vector<bls::G1Element> pks
        = adapters::convert_container<bls::G1Element>(public_keys, adapters::public_key_to_g1);
...
Signature Key::AggregateSignatures(std::vector<Signature> const& signatures)
{
    std::vector<bls::G2Element> sigs
        = adapters::convert_container<bls::G2Element>(signatures, adapters::signature_to_g2);
    auto agg_sig = bls::AugSchemeMPL().Aggregate(sigs);
    return utils::bytes_cast<SIG_LEN>(agg_sig.Serialize());
}

bool Key::AggregateVerifySignature(
    std::vector<PublicKey> const& public_keys, std::vector<Bytes> const& messages, Signature const& signature)
{
    std::vector<bls::G1Element> pks
        = adapters::convert_container<bls::G1Element>(public_keys, adapters::public_key_to_g1);
...
Key Key::GetWalletKey(uint32_t index, bool unhardened) const { return DerivePath({ 12381, 8444, 2, index }); }

Key Key::GetFarmerKey(uint32_t index, bool unhardened) const { return DerivePath({ 12381, 8444, 0, index }); }

Key Key::GetPoolKey(uint32_t index, bool unhardened) const { return DerivePath({ 12381, 8444, 1, index }); }

Key Key::GetLocalKey(uint32_t index, bool unhardened) const { return DerivePath({ 12381, 8444, 3, index }); }

Key Key::GetBackupKey(uint32_t index, bool unhardened) const { return DerivePath({ 12381, 8444, 4, index }); }

Address Key::GetAddress(std::string_view prefix) const
{
    auto puzzle_hash
        = utils::BytesToInts(utils::HashToBytes(puzzle::puzzle_for_public_key(GetPublicKey()).GetTreeHash()));
